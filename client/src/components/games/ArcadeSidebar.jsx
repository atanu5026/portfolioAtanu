  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed left-0 top-1/2 -translate-y-1/2 z-50 bg-zinc-950 border border-l-0 border-slate-900 p-4 rounded-r-xl ${hoverColor} transition-colors group flex flex-col items-center gap-2`}
      >
        ...
        {isOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed left-0 top-0 bottom-0 w-full max-w-md bg-zinc-950 border-r border-slate-900 z-50 shadow-2xl flex flex-col font-mono"
            >
              <div className="p-6 border-b border-slate-900 flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-bold tracking-widest text-slate-300 uppercase">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {games.map(game => (
                  <button
                    ...
                    className={`w-full text-left p-4 border border-slate-900 bg-black transition-all group ${game.component ? borderHover : 'opacity-50 cursor-not-allowed'}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <h3 className={`font-bold tracking-widest uppercase ${game.component ? 'text-white group-hover:' + accentColor.split(' ')[0] : 'text-slate-500'}`}>
                        {game.name}
                      </h3>
