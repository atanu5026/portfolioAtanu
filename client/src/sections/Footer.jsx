...
const Footer = () => {
  ...
  return (
    <footer className="w-full relative mt-32">
      ...
      <div className="w-full bg-zinc-950 border-t border-b border-slate-900 py-3 overflow-hidden flex items-center relative z-10">
        <div className={`absolute left-0 top-0 bottom-0 px-4 flex items-center z-20 bg-zinc-950 border-r ${bgAccent} font-mono text-xs tracking-widest font-bold uppercase`}>
          Live Server Log
        </div>
      ...
      <div className="relative bg-black pt-16 pb-8">
        <div className="container mx-auto px-4 md:px-12 flex flex-col md:flex-row justify-between items-center">
          <div className="mb-8 md:mb-0 text-center md:text-left">
            <h2 className="text-2xl font-bold tracking-widest uppercase text-white mb-2">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-8 text-[10px] md:text-xs font-mono tracking-widest font-bold">
            <a href={profileData?.socialLinks?.github || '#'} target="_blank" rel="noreferrer" className={`hover:${textColor} transition-colors text-slate-400`}>GITHUB</a>
            <a href={profileData?.socialLinks?.linkedin || '#'} target="_blank" rel="noreferrer" className={`hover:${textColor} transition-colors text-slate-400`}>LINKEDIN</a>
            <a href={profileData?.socialLinks?.email ? `mailto:${profileData.socialLinks.email}` : '#connect'} className={`hover:${textColor} transition-colors text-slate-400`}>EMAIL</a>
          </div>
        </div>
        
        <div className="container mx-auto px-4 md:px-12 mt-12 flex justify-center border-t border-slate-900 pt-8">
          <ThemePicker />
        </div>
